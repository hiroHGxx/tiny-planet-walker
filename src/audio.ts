...
/**
 * ループするBGMのメロディの台帳。notes は [MIDIノート番号, 拍数] の並びで0は休符、
 * beat は1拍の長さ(秒)。最後まで鳴らすと先頭に戻って無限にループする。
...
export interface AmbientAudio {
  ...
  /** BGMのメロディを切り替える(昼/夜/四季。次に予約する音から反映) */
...
export function createAmbientAudio(): AmbientAudio {
  ...
  const scheduleMelody = () => {
    if (!context) return;
    const { notes: melody, beat } = MELODIES[melodyMode];
