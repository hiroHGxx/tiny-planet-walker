...
import { HERBS, HERB_BY_ID } from './content/herbs.ts';
...
// 図鑑に記録する薬草の種類(ファクトリ→種類ID)。台帳(content/herbs.ts)から派生
const HERB_SPECIES_ID = new Map(HERBS.map((herb) => [herb.factory, herb.id]));

/** 種類IDから株の工場を引く(特定の薬草を狙って生やすとき用) */
const herbFactoryOf = (id: string) => HERB_BY_ID.get(id)!.factory;
...
// レイアウト計画(村・湖・道・畑などの設計図)は layout.ts に集約した。
// 参照元(features/content/テスト)のために公開定数はここからも提供する
import {
  START_NORMAL,
  SHOP_DIRECTION,
  SHOP_FRONT,
  HERB_CLUSTER_CENTERS,
  VILLAGE_CENTERS,
  LAKES,
  HILLS,
  FOREST_CENTERS,
...
export function createWorld(scene: THREE.Scene, planetIndex = 0): World {
  ...
  for (const field of FLOWER_FIELDS) {
    for (let i = 0; i < 26; i++) {
      const factory = herbFactoryOf(rand() < 0.6 ? 'smallflower' : 'starflower');
...
function addHerbClusters(
  ...
  // この星に生える種類(content/planets.ts)だけを台帳から選ぶ
  const available = PLANET_HERBS[currentPlanet()] ?? PLANET_HERBS[1]!;
  const herbFactories = HERBS.filter((herb) => available.includes(herb.id)).map(
    (herb) => herb.factory
  );
  for (const center of HERB_CLUSTER_CENTERS) {
    ...
    for (let i = 0; i < herbCount; i++) {
      ...
      scene.add(herb);
      if (factory === herbFactoryOf('glow')) glowHerbPositions.push(herb.position.clone());
      sightings.push({
    ...
    for (let i = 0; i < flowerCount; i++) {
      const flower = herbFactoryOf('smallflower')(rand);
...
function addShopArea(
  ...
  const herbFactories = ['roundleaf', 'starflower', 'glow', 'berry', 'smallflower', 'bud'].map(
    herbFactoryOf
  );
  for (let i = 0; i < 6; i++) {
    const factory = herbFactories[Math.floor(rand() * herbFactories.length)]!;
